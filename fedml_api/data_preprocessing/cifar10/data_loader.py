...
def partition_data( datadir, partition, n_nets, alpha, logger):
    ...
    if partition == 'n_cls':
        ...
        # upper = len(y_train) / n_client
        # random.seed(1)
        # n_data_per_clnt = [random.randint(200, upper) for _ in range(n_client)]
        # log_data=np.log(n_data_per_clnt)
        # mean = np.mean(log_data)
        # variance = np.var(log_data)
        # clnt_data_list = np.random.lognormal(mean=mean, sigma=variance, size=n_client)
        n_data_per_clnt = len(y_train) / n_client
        #n_data_per_clnt = 500
        clnt_data_list = np.random.lognormal(mean=np.log(n_data_per_clnt), sigma=0, size=n_client)
        clnt_data_list = (clnt_data_list / np.sum(clnt_data_list) * len(y_train)).astype(int)
        #clnt_data_list = (clnt_data_list / np.sum(clnt_data_list) * n_client*n_data_per_clnt).astype(int)
        print("clnt_data_list:{}".format(clnt_data_list))
        random.seed(1)
        ...
        while np.sum(clnt_data_list) != 0:
            ...
            # If current node is full resample a client
            # print('Remaining Data: %d' %np.sum(clnt_data_list))
            #print("curr_clnt:{}, clnt_data_list[curr_clnt]:{}".format(curr_clnt, clnt_data_list[curr_clnt]))
            if clnt_data_list[curr_clnt] <= 0:
                continue
            clnt_data_list[curr_clnt] -= 1
            curr_prior = prior_cumsum[curr_clnt]
            #print("clnt_data_list[curr_clnt]:{},curr_prior:{}".format(clnt_data_list[curr_clnt],curr_prior))
            while True:
                cls_label = np.argmax(np.random.uniform() <= curr_prior)
                # Redraw class label if trn_y is out of that class
                if cls_amount[cls_label] <= 0:
