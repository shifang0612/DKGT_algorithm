...
from fedml_api.standalone.DFL import client
from fedml_api.standalone.DFL.client import Client
...
class dflAPI(object):
    ...
    def train(self):
        ...
        zero_model = {}
        for name, params in w_global.items():
            zero_model[name] = torch.zeros_like(params)
        random.seed(1)
        ...
        for round_idx in range(self.args.comm_round):
            ...
            round_m=math.floor(round_idx/self.args.tau)
            ...
            weight_matrix, neighbor_matrix = self.obtain_weight_matrix(topo_matrix,self.args.client_num_in_total)
            ...
            for clnt_idx in range(self.args.client_num_in_total):
                ...
                w_local_mdl = self._aggregate_func(clnt_idx, neighbor_matrix[clnt_idx], weight_matrix, w_local_mdls_lstrd)

                # 设置client进行local training
                client = self.client_list[clnt_idx]
                w_trained_mdl, tst_results, gradient = client.train(w_local_mdl, round_idx)
                #self.logger.info("client id:{}, local gradient:{}".format(clnt_idx, gradient))
    ...
    def _aggregate_func(self, cur_idx, nei_indexs, agg_weight, w_trained_mdls):
        ...
        w_tmp = copy.deepcopy(w_trained_mdls[cur_idx])
        for k in w_tmp.keys():
            w_tmp[k] = w_tmp[k] - w_tmp[k]
            for clnt in nei_indexs:
                w_tmp[k] += agg_weight[cur_idx][clnt] * w_trained_mdls[clnt][k]
        return w_tmp
